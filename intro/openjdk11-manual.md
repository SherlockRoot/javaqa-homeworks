...
Откройте терминал и проверьте что после установки Java работает без указания полного пути с помощью следующей команды:

```shell script
java -version
```
Вывод проверки текущей версии должен выглядеть следующим образом:
```
