...
Откройте cmd (командная строка) и проверьте что после установки Java работает без указания полного пути с помощью следующей команды:
Нажмите Windows + R, чтобы открыть окно «Выполнить». Введите «cmd», а затем нажмите «ОК»
Вы можете легко открыть командную строку, нажав «Пуск», а затем введя «cmd» в поле поиска. Либо щелкните/коснитесь значка микрофона в поле поиска Cortana и произнесите «Запустить командную строку».
